    ...
    <Page size={"A4"} style={tw(`p-8 pt-12 font-[Lora] text-[11pt]`)}>
      ...
      <View style={tw(" -mt-2 text-center px-2")}>
        <Html>
          ...
          ${ReactDOMServer.renderToStaticMarkup(parse(payload.summary))}
          `}
        </Html>
      </View>
      ...
      <View style={tw("px-2")}>
        {payload.workExperiences.map((el) => (
          <View style={tw("mb-2")}>
            <View
              style={tw(
          ...
          ${ReactDOMServer.renderToStaticMarkup(parse(el.description))}
          `}
            </Html>
          </View>
        ))}
