...
function Calendar({
  ...
  yearRange = 50,
  presentStateChecked = false,
  ...props
}: CalendarProps & { yearRange?: number; presentStateChecked?: boolean }) {
  ...
  return (
    <DayPicker
      disabled={presentStateChecked}
      showOutsideDays={showOutsideDays}
      className={cn(
        `p-3 ${
          presentStateChecked && " pointer-events-none cursor-not-allowed"
        }`,
        className
      )}
      classNames={{
...
export const TIME_PRESENT = new Date("1000-10-10");
...
>(
  (
    ...
    const [presentStateChecked, setPresentStateChecked] = React.useState(false);

    const handleChecked = (val: boolean) => {
      setPresentStateChecked(val);
      if (val) {
        callback(TIME_PRESENT.toDateString());
      }
    };

    const handleDateShow = (value: Date | undefined) => {
      if (value) {
        if (value.toDateString() == TIME_PRESENT.toDateString()) {
      ...
      <Popover>
        <PopoverTrigger asChild disabled={disabled}>
          <Button
            variant="outline"
            className={cn(
              "w-full justify-start text-left font-normal bg-neutral-200 hover:bg-neutral-200",
        ...
        <PopoverContent className="w-auto p-0">
          <Calendar
            presentStateChecked={presentStateChecked}
            disabled={presentStateChecked}
            mode="single"
            selected={value}
            month={month}
